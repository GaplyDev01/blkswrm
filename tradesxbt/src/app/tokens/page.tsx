              ...
              <tbody>
                {filteredTokens.map((token) => (
                  <tr key={token.id} className="border-b border-border hover:bg-card">
                    ...
                    <td className="p-4 text-right font-medium">${token.price.toLocaleString(undefined, { minimumFractionDigits: token.price < 0.01 ? 6 : 2, maximumFractionDigits: token.price < 0.01 ? 6 : 2 })}</td>
                    <td className={`p-4 text-right ${token.change24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                      {token.change24h >= 0 ? '+' : ''}{token.change24h.toFixed(2)}%
                    </td>
                    <td className="p-4 text-right hidden md:table-cell">${(token.volume24h / 1000000).toFixed(2)}M</td>
                    <td className="p-4 text-right hidden md:table-cell">${(token.marketCap / 1000000000).toFixed(2)}B</td>
                  </tr>
                ))}
              </tbody>
            </table>
