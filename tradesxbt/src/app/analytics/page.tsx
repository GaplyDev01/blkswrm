        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400">
              Welcome, {user?.firstName || user?.username || 'Trader'}. View your performance metrics.
            </p>
          </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { token: 'BONK', type: 'sell', amount: 5000, price: 0.00001547, time: '5h ago' },
                    { token: 'JUP', type: 'buy', amount: 12.5, price: 1.24, time: '1d ago' },
                  ].map((trade, index) => (
                    <div key={index} className="flex justify-between items-center bg-black/20 rounded-md p-2">
                      <div className="flex items-center space-x-2">
                        <div className={`px-2 py-1 rounded text-xs ${
                          trade.type === 'buy' ? 'bg-green-900/30 text-green-500' : 'bg-red-900/30 text-red-500'
                        }`}>
                          {trade.type.toUpperCase()}
                        </div>
                        <div className="text-white">{trade.token}</div>
                      </div>
                      <div className="text-sm text-gray-400">{trade.amount} @ ${trade.price < 0.01 ? trade.price.toFixed(8) : trade.price.toFixed(2)}</div>
