...
import { formatCurrency, formatNumber, formatPercentage, getPriceChangeColor, shortenAddress, formatTimestamp } from "@/lib/utils";
            ...
            >
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
                <div className="flex items-center mb-4 md:mb-0">
                  ...
                  <div>
                    <h2 className="text-xl font-medium">{connected ? "Wallet Connected" : "Connect Wallet"}</h2>
                    {connected && walletAddress && (
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="border-b border-white/10">
                              <th className="text-left p-3 text-sm text-gray-400 font-medium">Token</th>
                              <th className="text-right p-3 text-sm text-gray-400 font-medium">Balance</th>
                              <th className="text-right p-3 text-sm text-gray-400 font-medium">Value</th>
                              <th className="text-right p-3 text-sm text-gray-400 font-medium">Actions</th>
                            </tr>
                          </thead>
                          <tbody>
                            {balances.map((token) => (
                              <tr key={token.symbol} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                ...
                                <td className="p-3 text-right">
                                  {token.amount < 0.01 ? formatNumber(token.amount, 8) : formatNumber(token.amount)}
                                </td>
                                <td className="p-3 text-right">
                    ...
                    {txLoading ? (
                      ...
                      <div className="overflow-x-auto -mx-5 px-5">
                        <table className="w-full">
                          <thead>
                            <tr className="border-b border-white/10">
                              <th className="text-left p-3 text-sm text-gray-400 font-medium">Type</th>
                              <th className="text-left p-3 text-sm text-gray-400 font-medium">Details</th>
                              <th className="text-right p-3 text-sm text-gray-400 font-medium">Amount</th>
                              <th className="text-right p-3 text-sm text-gray-400 font-medium">Time</th>
                              <th className="text-right p-3 text-sm text-gray-400 font-medium">Status</th>
                            </tr>
                          </thead>
                          <tbody>
                            {transactions.map((tx) => (
                              <tr key={tx.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                ...
                                <td className="p-3 text-right">
                                  {tx.type === 'swap' ? (
                                    ...
                                    <span className="text-red-400">-{tx.amount} {tx.token}</span>
                                  )}
                                </td>
                                ...
                                <td className="p-3 text-right">
                                  <Badge variant="outline" className="bg-green-900/20 text-green-400 border-green-500/20">
              ...
              <CryptoCard variant="glass" className="text-center py-16">
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-gray-400 mb-6">
                    Connect your Solana wallet to start trading, viewing your portfolio, and managing your assets.
                  </p>
                  <Button 
                    onClick={handleConnectWallet} 
                    disabled={loading}
