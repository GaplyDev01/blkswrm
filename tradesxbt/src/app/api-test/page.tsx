...
import { useMarketData, useTrendingTokens, useTokenData, useTokenSearch } from '@/hooks/useMarketData';
...
export default function ApiTestPage() {
  ...
  const { data: trendingData, error: trendingError, isLoading: trendingLoading } = useTrendingTokens();
  const { data: tokenData, error: tokenError, isLoading: tokenLoading } = useTokenData(selectedToken, { days: parseInt(chartDays, 10) });
  const { data: searchData, error: searchError, isLoading: searchLoading } = useTokenSearch({ query: searchQuery });
                ...
                {searchData.results.map((token: any) => (
                  ...
                  >
                    <div className="flex items-center">
                      <img src={token.thumb} alt={token.name} className="w-8 h-8 rounded-full mr-3" />
                      <div>
                        <div className="font-medium">{token.name}</div>
                        <div className="text-xs text-gray-400">{token.symbol}</div>
                      </div>
              ...
              <div>
                <div className="flex items-center mb-4">
                  {tokenData.image?.small && (
                    <img src={tokenData.image.small} alt={tokenData.name} className="w-10 h-10 rounded-full mr-3" />
                  )}
                  <div>
                    <h3 className="text-lg font-medium">{tokenData.name} ({tokenData.symbol})</h3>
                    <div className="flex items-center text-sm">
                      <span className="text-gray-400 mr-2">Price:</span>
                      <span className="font-medium">${tokenData.market_data?.current_price?.toFixed(4)}</span>
                      <span className={`ml-2 ${tokenData.market_data?.price_change_percentage_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="bg-black/20 p-3 rounded-lg">
                    ...
                    <div className="font-medium">${tokenData.market_data?.market_cap?.toLocaleString()}</div>
                  </div>
                  <div className="bg-black/20 p-3 rounded-lg">
                    <div className="text-sm text-gray-400">Volume (24h)</div>
                    <div className="font-medium">${tokenData.market_data?.total_volume?.toLocaleString()}</div>
                  </div>
                  <div className="bg-black/20 p-3 rounded-lg">
                    <div className="text-sm text-gray-400">Circulating Supply</div>
                    <div className="font-medium">{tokenData.market_data?.circulating_supply?.toLocaleString()} {tokenData.symbol}</div>
                  </div>
                </div>
                ...
                <div className="bg-black/20 p-3 rounded-lg mb-4">
                  ...
                  <div className="text-sm max-h-24 overflow-y-auto">
                    {tokenData.description ? (
                      <div dangerouslySetInnerHTML={{ __html: tokenData.description.substring(0, 300) + '...' }} />
                    ) : (
              ...
              <div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                  <div className="bg-black/20 p-3 rounded-lg">
                    ...
                    <div className="font-medium">${marketData?.metrics?.total_market_cap?.toLocaleString()}</div>
                  </div>
                  <div className="bg-black/20 p-3 rounded-lg">
                    <div className="text-sm text-gray-400">24h Volume</div>
                    <div className="font-medium">${marketData?.metrics?.total_volume_24h?.toLocaleString()}</div>
                  </div>
                  <div className="bg-black/20 p-3 rounded-lg">
                    <div className="text-sm text-gray-400">Gainers</div>
                    <div className="font-medium text-green-400">{marketData?.metrics?.positive_performers_24h}</div>
                  </div>
                  <div className="bg-black/20 p-3 rounded-lg">
                    <div className="text-sm text-gray-400">Losers</div>
                    <div className="font-medium text-red-400">{marketData?.metrics?.negative_performers_24h}</div>
                  </div>
                </div>
                  ...
                  <div className="bg-black/20 p-3 rounded-lg">
                    ...
                    {marketData?.metrics?.best_performer ? (
                      <div className="flex items-center">
                        ...
                        />
                        <div>
                          <div className="font-medium">{marketData.metrics.best_performer.name}</div>
                          <div className="flex items-center">
                            <span className="text-sm">${marketData.metrics.best_performer.price.toFixed(4)}</span>
                            <span className="text-xs text-green-400 ml-2">+{marketData.metrics.best_performer.change.toFixed(2)}%</span>
                          </div>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className="bg-black/20 p-3 rounded-lg">
                    ...
                    {marketData?.metrics?.worst_performer ? (
                      <div className="flex items-center">
                        ...
                        />
                        <div>
                          <div className="font-medium">{marketData.metrics.worst_performer.name}</div>
                          <div className="flex items-center">
                            <span className="text-sm">${marketData.metrics.worst_performer.price.toFixed(4)}</span>
                            <span className="text-xs text-red-400 ml-2">{marketData.metrics.worst_performer.change.toFixed(2)}%</span>
                          </div>
                        </div>
                      </div>
                    ) : (
