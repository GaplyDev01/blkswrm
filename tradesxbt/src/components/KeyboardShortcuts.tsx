...
export default function KeyboardShortcuts({ 
  ...
  const groupedShortcuts = shortcuts.reduce((acc: {[key: string]: KeyboardShortcut[]}, shortcut) => {
      ...
      <div className="bg-black/90 border border-[#00FF80]/20 rounded-lg w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    {groupShortcuts.map((shortcut, i) => (
                      <div key={i} className="flex justify-between items-center">
                        ...
                        <div className="flex space-x-1">
                          {shortcut.ctrlKey && (
                            <Badge variant="outline" className="bg-black/40 border-white/20">Ctrl</Badge>
                          )}
                          {shortcut.altKey && (
                            <Badge variant="outline" className="bg-black/40 border-white/20">Alt</Badge>
                          )}
                          {shortcut.shiftKey && (
                            <Badge variant="outline" className="bg-black/40 border-white/20">Shift</Badge>
                          )}
        ...
        <div className="p-3 border-t border-[#00FF80]/20 bg-black/40 flex justify-between items-center">
          <div className="text-xs text-gray-400">
            Press <kbd className="px-1 py-0.5 bg-black/40 border border-white/10 rounded">?</kbd> anytime to show this dialog
          </div>
          <Button 
            variant="outline" 
            size="sm" 
