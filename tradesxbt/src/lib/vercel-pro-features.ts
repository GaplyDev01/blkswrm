...
export async function loadVercelProConfig(): Promise<VercelProConfig> {
  try {
    ...
    const config = await fetchConfiguration('/api/vercel-config');
    return { ...defaultConfig, ...config };
  } catch (error) {
    console.error('Error loading Vercel Pro configuration:', error);
  ...
  return {
    securityHeaders: {
      contentSecurityPolicy: {
        directives: {
          defaultSrc: ["'self'"],
          scriptSrc: ["'self'", "'unsafe-eval'", "'unsafe-inline'", 'vercel.live', '*.vercel-scripts.com'],
